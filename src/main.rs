...
impl AddParam {
  fn new() -> Self {
    Self { title: None, authors: None, file: None }
  }
}

fn read_json_field<T>(field: Field) -> impl Future<Item=T, Error=error::Error> where for <'a> T: serde::Deserialize<'a> {
  field.map_err(|e| {
    ...
    serde_json::from_slice(&body).map_err(|e| error::ErrorInternalServerError(e))
  })
}
...
fn read_file_field(field: Field) -> impl Future<Item=NamedTempFile, Error=error::Error> {
  ...
  future::Either::B(field.fold(file, |mut file, chunk| {
    web::block(move || {
...
fn add(data: web::Data<Arc<AppData>>, multipart: Multipart) -> impl Future<Item=impl Responder, Error=error::Error> {
  multipart.map_err(|e| error::ErrorInternalServerError(e))
   .fold(AddParam::new(), |mut param, field| -> Box<dyn Future<Item=_, Error=_>> {
    let cd = match field.content_disposition() {
      Some(cd) => cd,
      None => return box future::err(error::ErrorInternalServerError(format_err!("The content disposition header is required."))),
    };
    let name = match cd.get_name() {
      Some(name) => name,
      None => return box future::err(error::ErrorInternalServerError(format_err!("The name is required."))),
    };
    match name {
    ...
    let title = match param.title {
      Some(val) => val,
      None => return Err(error::ErrorInternalServerError(format_err!("'title' is not provided"))),
    };
    let authors = match param.authors {
      Some(val) => val,
      None => return Err(error::ErrorInternalServerError(format_err!("'authors' is not provided"))),
    };
    let file = match param.file {
      Some(val) => val,
      None => return Err(error::ErrorInternalServerError(format_err!("'file' is not provided"))),
    };
    Ok((title, authors, file))
  }).and_then(move |(title, authors, file)| {
    data.db.send(db::Add::new(title, authors))
    .then(|result| {
      let article = result??;
      file.persist(article.path())
